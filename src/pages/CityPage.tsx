...
const CITIES: Record<string, { state: string; population: string; parkingNotable: string }> = {
...
const CityPage = () => {
  ...
  const city = cityName ? CITIES[cityName.toLowerCase()] : undefined;
  const displayName = cityName ? cityName.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ') : '';

  if (!city || !cityName) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-page">
        <div className="text-center">
          <p className="font-body text-body text-lg mb-4">City guide not found.</p>
  ...
  const canonicalSlug = cityName.toLowerCase();
  const title = `Protect Your Car at Valet in ${displayName}, ${city.state} — CarShake`;
  const metaDescription = `AI-verified car scans before & after valet parking in ${displayName}. 60-second QR handover proof. Free, no app download. Trusted by ${displayName} drivers.`;
  const h1 = `Protect Your Car at Valet Parking in ${displayName}`;
  ...
  const jsonLd = {
    ...
    areaServed: {
      '@type': 'City',
      name: displayName,
      sameAs: `https://en.wikipedia.org/wiki/${encodeURIComponent(displayName.replace(/-/g, ' '))}`,
    },
    url: `https://carshake.online/city/${canonicalSlug}`,
  };
  ...
  return (
    <div className="min-h-screen bg-page">
      <Helmet>
        ...
        <link rel="canonical" href={`https://carshake.online/city/${canonicalSlug}`} />
      ...
      <header className="px-4 py-4 border-b border-border bg-white">
        <div className="max-w-[720px] mx-auto flex items-center gap-4">
          ...
          <Link to="/city" className="text-sm font-body text-muted-custom hover:text-gold transition">← City Guides</Link>
        </div>
      </header>
      ...
      <main className="max-w-[720px] mx-auto px-4 py-10">
        ...
        <h1 className="font-display text-[28px] font-bold text-ink mb-6 leading-tight">{h1}</h1>

        <p className="font-body text-[15px] text-body leading-relaxed mb-5">
          {displayName}, {city.state} is {city.parkingNotable}. When you hand your keys to a valet, you're trusting someone with a vehicle worth thousands of dollars. Without documented proof of your car's condition before the handover, you have no defense if damage appears at pickup.
        </p>

        <p className="font-body text-[15px] text-body leading-relaxed mb-5">
          CarShake creates a signed, timestamped, AI-verified record of your car's condition in 60 seconds — right from your phone, no app download needed. The parking attendant scans your QR code and confirms. Both sides sign. Both sides are protected.
        </p>

        <p className="font-body text-[15px] text-body leading-relaxed mb-5">
          With a population of {city.population}, {displayName} has thousands of daily valet interactions. CarShake gives {displayName} drivers the peace of mind that every scratch, dent, and ding is documented before anyone else touches your car.
        </p>

        {/* CTA */}
        <div className="my-10 p-6 rounded-[14px] border-2 border-gold bg-gold-subtle text-center">
          <p className="font-display text-lg font-bold text-ink mb-2">Try CarShake in {displayName} — it's free.</p>
          ...
          <a
            ...
            onClick={() => track('cta_clicked', { location: 'city-page', city: canonicalSlug })}
          >
        ...
        {/* Related use cases */}
        <h2 className="font-display text-xl font-bold text-ink mb-4">Valet Scenarios in {displayName}</h2>
        <div className="space-y-3 mb-10">
          <Link to="/protect/valet-parking" className="block bg-white rounded-[14px] border border-border p-4 hover:shadow-lg transition">
            <h3 className="font-display text-[15px] font-bold text-ink">Valet Parking Protection</h3>
            <p className="font-body text-sm text-body mt-1">How to document your car before handing keys to a valet attendant</p>
          </Link>
          <Link to="/protect/hotel-parking" className="block bg-white rounded-[14px] border border-border p-4 hover:shadow-lg transition">
            <h3 className="font-display text-[15px] font-bold text-ink">Hotel Parking Protection</h3>
            <p className="font-body text-sm text-body mt-1">Protect your car during hotel stays and overnight valet parking</p>
          </Link>
          <Link to="/protect/airport-parking" className="block bg-white rounded-[14px] border border-border p-4 hover:shadow-lg transition">
            <h3 className="font-display text-[15px] font-bold text-ink">Airport Parking Protection</h3>
            <p className="font-body text-sm text-body mt-1">Document your car before long-term airport parking in {displayName}</p>
          </Link>
        </div>

        <div className="text-center">
          <Link to="/city" className="text-gold font-body font-semibold text-sm">← All City Guides</Link>
        </div>
      </main>
