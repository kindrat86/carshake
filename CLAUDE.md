...
- **РОЗВʼЯЗКА SPLIT-BRAIN 2026-07-31: carshake.online живе на Vercel-проєкті `carshake-landing`, НЕ `carshake` і НЕ `carshake-prod`.** `.vercel/project.json` був тихо злінкований на `carshake-prod` — деплої туди аліасяться лише на carshake-prod.vercel.app, тож "успішний" `vercel --prod` НЕ доходив до домену. Перелінковано (`vercel link --project carshake-landing`). Проєкт мав framework=nextjs у налаштуваннях → `npm run vercel-build` падав на статичному сайті; знято через `PATCH /v9/projects/carshake-landing {"framework":null}`. Після БУДЬ-ЯКОГО деплою: `vercel inspect <deployment-url>` і переконайся, що в Aliases є carshake.online
- **ПРОТОКОЛ ПЕРЕЙМЕНУВАННЯ ВИПРАВЛЕНО 2026-07-31 — "заміни імʼя в інших chunk-ах" (на місці) ЛАМАЄ повторних відвідувачів.** Кешований на рік sibling-chunk імпортує СТАРУ назву entry, свіжий HTML вантажить НОВУ → два інстанси React → `useAuth must be used within AuthProvider`, порожній застосунок (спостережено наживо). Правильно — **generational split**: (1) перейменуй КОЖЕН chunk, чий вміст змінився (включно з ~31 sibling-ами, що імпортують entry); (2) нове покоління посилається ТІЛЬКИ на нові назви; (3) ВІДНОВИ старі файли byte-identical під старими назвами (`git show HEAD~1:path > path`) — кешовані HTML/chunk-и резолвляться в робочий старий застосунок, поки кеш згасає; (4) vendor-chunk-и (charts/supabase/markdown) спільні для обох поколінь — перевір, що жоден незмінений chunk не імпортує змінений sibling і жоден HTML не посилається на sibling-назви напряму
