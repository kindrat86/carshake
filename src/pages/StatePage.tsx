...
const STATES: Record<string, { name: string; bailmentLaw: string; statuteOfLimitations: string; notableCase: string }> = {
...
const StatePage = () => {
  ...
  const data = stateName ? STATES[stateName.toLowerCase()] : undefined;
  const displayName = stateName ? stateName.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ') : '';

  if (!data || !stateName) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-page">
        <div className="text-center">
          <p className="font-body text-body text-lg mb-4">State guide not found.</p>
  ...
  const canonicalSlug = stateName.toLowerCase();
  const title = `Valet Parking Liability in ${displayName} — CarShake Legal Guide`;
  const metaDescription = `Learn about valet parking bailment laws, liability limits, and statute of limitations in ${data.name}. Protect your car with AI-verified documentation.`;
  ...
  return (
    <div className="min-h-screen bg-page">
      <Helmet>
        ...
        <link rel="canonical" href={`https://carshake.online/state/${canonicalSlug}`} />
      </Helmet>
      ...
      <main className="max-w-[720px] mx-auto px-4 py-10">
        <h1 className="font-display text-[28px] font-bold text-ink mb-4 leading-tight">
          Valet Parking Liability Laws in {displayName}
        </h1>

        <div className="space-y-5 mb-8">
          <div className="bg-white rounded-[14px] border border-border p-5">
            <h2 className="font-display text-base font-bold text-ink mb-2">🔑 Governing Bailment Law</h2>
            <p className="font-body text-sm text-body">{data.bailmentLaw}</p>
          </div>

          <div className="bg-white rounded-[14px] border border-border p-5">
            <h2 className="font-display text-base font-bold text-ink mb-2">⏱️ Statute of Limitations</h2>
            <p className="font-body text-sm text-body">You have <strong>{data.statuteOfLimitations}</strong> to file a claim for valet parking damage in {displayName}.</p>
          </div>

          <div className="bg-white rounded-[14px] border border-border p-5">
            <h2 className="font-display text-base font-bold text-ink mb-2">⚖️ Notable Application</h2>
            <p className="font-body text-sm text-body">{data.notableCase}</p>
          </div>
        </div>

        <div className="bg-gold-subtle border-2 border-gold rounded-[14px] p-6 mb-8">
          <h2 className="font-display text-lg font-bold text-ink mb-3">Why evidence matters in {displayName}</h2>
          <p className="font-body text-sm text-body mb-4">
            Legal rights don't matter unless you can prove when damage occurred. CarShake creates GPS-verified, timestamped, SHA-256 hashed evidence of your car's condition before and after valet parking — admissible evidence that changes the outcome of disputes. Even with strong {displayName} bailment laws, without documentation, it's your word against the parking operator's.
          </p>
          <a
            href="/#demo"
            ...
            onClick={() => track('cta_clicked', { location: 'state-page', state: canonicalSlug })}
          >
            🛡️ Get Proof Before You Park — Free
          </a>
        </div>
